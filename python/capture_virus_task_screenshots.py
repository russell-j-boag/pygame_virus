...
FEEDBACK_EXAMPLES = {
    "CAL_LP": ("CORRECT", task.FEEDBACK_CORRECT_COLOR),
    "M_HP": ("INCORRECT", task.FEEDBACK_ERROR_COLOR),
    "A_HP": ("TOO SLOW", task.FEEDBACK_SLOW_COLOR),
    "M_LP": ("CORRECT", task.FEEDBACK_CORRECT_COLOR),
    "A_LP": ("INCORRECT", task.FEEDBACK_ERROR_COLOR),
}
...
def condition_code(block_cfg: dict) -> str:
    return task.block_condition_deadline_code(block_cfg)
...
def block_slider_items(block_name: str) -> list[dict]:
    ...
    if block_name in ("AUTOMATION", "AUTOMATION1", "AUTOMATION2"):
...
def draw_example_trial(surface, fonts, keymap, block_cfg):
    ...
    ui_payload = {
        ...
        "trials_left": block_cfg["N_TRIALS"],
...
def render_block(writer, surface, fonts, keymap, block_cfg):
    ...
    draw_example_trial(surface, fonts, keymap, block_cfg)
    ...
    if block_cfg["name"] in ("AUTOMATION", "AUTOMATION1", "AUTOMATION2"):
